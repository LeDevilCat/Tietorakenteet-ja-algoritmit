...
public:
	...
	void tri_activity(int time, Plane &landing_plane, Plane &takeoff_plane, Plane &flex_plane, bool &land_processed, bool &takeoff_processed, bool &flex_processed) {
		land_processed = false;
		takeoff_processed = false;
		flex_processed = false;

		// Runway 1: Always dedicated to landings
		if (!landing_queue.empty()) {
		...
		// Runway 2: Always dedicated to takeoffs
		if (!takeoff_queue.empty()) {
		...
		// Runway 3: Flexible runway
		// Primarily for landings, but used for takeoffs if landing queue is empty
		if (!landing_queue.empty()) {
			landing_queue.retrieve(flex_plane);
			land_wait += time - flex_plane.started();
			num_landings++;
			landing_queue.serve();
			flex_processed = true;
		} else if (!takeoff_queue.empty()) {
			takeoff_queue.retrieve(flex_plane);
			takeoff_wait += time - flex_plane.started();
			num_takeoffs++;
			takeoff_queue.serve();
			flex_processed = true;
		} else {
	...
	void shut_down(int time) const {
		...
		double flex_idle_percent = (time > 0) ? (100.0 * static_cast<double>(flex_idle_time) / static_cast<double>(time)) : 0.0;
		...
		cout << "Percentage of time flexible runway idle " << flex_idle_percent << "%" << endl;
...
void initialize(int &end_time, int &queue_limit, double &arrival_rate, double &departure_rate) {
	cout << "This program simulates an airport with three runways." << endl
		 << "Runway 1 is always dedicated to landings." << endl
		 << "Runway 2 is always dedicated to takeoffs." << endl
		 << "Runway 3 is reserved for landings, unless the landing queue is empty," << endl
...
void P4() {
	...
	p4_sim::initialize(end_time, queue_limit, arrival_rate, departure_rate);

	Random variable(false);
	p4_sim::Airport airport(queue_limit);

	for (int current_time = 0; current_time < end_time; ++current_time) {
		int number_arrivals = variable.poisson(arrival_rate);
		for (int i = 0; i < number_arrivals; ++i) {
			p4_sim::Plane current_plane(flight_number++, current_time, p4_sim::arriving);
			if (airport.can_land(current_plane) != p4_sim::success) {
		...
		for (int j = 0; j < number_departures; ++j) {
			p4_sim::Plane current_plane(flight_number++, current_time, p4_sim::departing);
			if (airport.can_depart(current_plane) != p4_sim::success) {
				current_plane.refuse();
			}
		}

		// Three runways: one landing, one takeoff, one flexible
		p4_sim::Plane landing_plane;
		p4_sim::Plane takeoff_plane;
		p4_sim::Plane flex_plane;
		bool land_processed, takeoff_processed, flex_processed;
		airport.tri_activity(current_time, landing_plane, takeoff_plane, flex_plane, land_processed, takeoff_processed, flex_processed);
		...
		if (flex_processed) {
			if (flex_plane.started() > current_time - flex_plane.started()) {
				flex_plane.land(current_time);
			} else {
				flex_plane.fly(current_time);
			}
		}
	}
